...
tty_t fbcon_tty = {.echo = fbcon_echo, .write = fbcon_write, .termios = {.c_lflag = ICANON | ECHO | ECHOE}, .name = "tty1"};
int fbcon_tty_id;
...
void fbcon_init() {
    ...
    fbcon_tty_id = register_tty(&fbcon_tty);
}
...
void fbcon_putchar(const char c) {
